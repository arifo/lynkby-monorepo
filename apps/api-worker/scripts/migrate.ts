...
const SCHEMA = "../../packages/server/prisma/schema.prisma";
const MIGRATION_COMMANDS = {
  ...
  RESET: `npx prisma migrate reset --force --schema ${SCHEMA}`,
  STATUS: `npx prisma migrate status --schema ${SCHEMA}`,
...
switch (command) {
  ...
  case "status":
    ...
    break;
  case "studio":
    console.log("🖥️  Launching Prisma Studio...");
    execSync(MIGRATION_COMMANDS.STUDIO, { stdio: "inherit" });
    break;
